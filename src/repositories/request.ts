...
export class RequestSingleton {
  ...
  private config: CustomAxiosRequestConfig = {
    headers: {},
  };

  private cbSuccess?: <T = any>(data: T, headers: any) => void;
  private cbFailure?: (error: { message: string; status?: number; data?: any }) => void;

  constructor(endpoint: string, method: string, defaults?: Partial<CustomAxiosRequestConfig>) {
    this.config = {
      url: endpoint,
      method,
      withAuth: true,
  ...
  public setBody(body: Record<string, any>, type: "json" | "formData" = "json", removeKeys: string[] = []): this {
    this.config.data = type === "json" ? body : RequestSingleton.jsonToFormData(body, removeKeys);
  ...
  public onFailure(cb: (error: { message: string; status?: number; data?: any }) => void): this {
    this.cbFailure = cb;
    return this;
  }
  ...
  public async call(): Promise<void> {
    try {
      ...
      const response: AxiosResponse = await RequestSingleton.instance.request(this.config);
  ...
  private static jsonToFormData(jsonObject: Record<string, any>, removeKeys: string[] = []): FormData {
...
export const request = (endpoint: string, method: string): RequestSingleton => new RequestSingleton(endpoint, method);
